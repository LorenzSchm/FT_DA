...
function SuccessToast({ text1, text2 }: BaseToastProps) {
  return (
    <View
      className="bg-black rounded-2xl mx-4 px-5 py-4 flex-row items-center shadow-lg"
      style={{ gap: 12 }}
    >
      <CheckCircle size={22} color="#4ade80" />
      <View className="flex-1">
        {text1 ? (
          <Text className="text-white font-semibold text-[15px]">{text1}</Text>
        ) : null}
        {text2 ? (
          <Text className="text-gray-400 text-[13px] mt-0.5">{text2}</Text>
        ) : null}
      </View>
    </View>
  );
