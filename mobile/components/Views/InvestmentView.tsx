...
export function InvestmentView() {
    ...
    const [portfolioHistory, setPortfolioHistory] = useState<HistoryMap | null>(null);
    const [chartLoading, setChartLoading] = useState(false);
    const [positionValues, setPositionValues] = useState<{ ticker: string; value: number }[]>([]);
        ...
        const fetchpositions = async () => {
            try {
                ...
                const vals = (posData || []).map((p: any) => {
                    ...
                    const fallbackQty = Number(p?.quantity ?? last?.position_quantity ?? 0) || 0;
                    const fallbackPrice = Number(p?.current_price ?? last?.current_price ?? 0) || 0;
                    const currentValue = Number(
                        (last?.market_value ?? (fallbackQty * fallbackPrice)) as number,
                    );
                    return {ticker: p.ticker, value: Number.isFinite(currentValue) ? currentValue : 0};
                });
                setPositionValues(vals);
                try {
                    console.log("positionValues", vals);
                } catch {
                }
            } catch (error) {
            ...
            try {
                ...
                tfs.forEach((tf) => {
                    ...
                    const perTicker = uniqueTickers.map((ticker) => {
                        ...
                        const firstTs = firstTradeTsByTicker[ticker] ?? Number.POSITIVE_INFINITY;
                        return {ticker, list, idx: 0, lastPrice: undefined as number | undefined, steps, firstTs};
                    });
                    ...
                    for (const ts of unionTimestamps) {
                        ...
                        for (const state of perTicker) {
                            ...
                            while (state.idx < list.length && list[state.idx].timestamp <= ts) {
                ...
                const globalFirstTs = (() => {
                    const vals = Object.values(firstTradeTsByTicker || {}).filter(
                        (v) => Number.isFinite(v),
                    ) as number[];
                ...
                const trimSeries = (arr: Point[]): Point[] => {
                    ...
                    let i = 0;
                    while (i < out.length && (!Number.isFinite(out[i].value) || out[i].value <= 0)) {
                        i++;
                    }
                    out = out.slice(i);
                    return out;
        ...
        <GestureHandlerRootView style={{flex: 1}}>
            <View className="pt-4">
                {chartLoading ? (
                    ...
                    <Text className="text-gray-500">No chart data</Text>
                )}
            </View>
            <View className="px-8">
                <View>
                                        ...
                                        <View className="flex-row items-center">
                                            ...
                                            <View>
                                                ...
                                                <Text className="font-semibold text-gray-500">
                                                    ${(() => {
                                                    const found = positionValues.find((v) => v.ticker === item.ticker);
