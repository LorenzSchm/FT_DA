...
export default function DashBoard() {
    ...
    const fetchConnectBalanceMinor = async () => {
        ...
        const connectData = await getData(session.access_token, session.refresh_token);
    ...
    const normalizeTransactions = (list?: any[]) => {
        ...
        return list.map((t: any) => {
            ...
            return {
                id: t.id ?? t.transaction_id ?? `${amountMinor}_${t.date ?? t.timestamp ?? ""}`,
                description:
                    t.description || t.merchant || t.name || t.merchant_name || "Transaction",
                category_id:
                    t.category || t.category_id || t.category_name || t.categoryLabel || "Other",
    ...
    const loadTransactionsForAccount = async (accountId: number, accountObj?: any) => {
        if (!session?.access_token || !accountId) return;
        try {
            setLoadingTxByAccount((prev) => ({...prev, [accountId]: true}));
            const matchingAccount = accountObj ?? accounts.find((acc) => acc.id === accountId);
