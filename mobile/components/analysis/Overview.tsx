...
export default function Overview({account, accounts}: Props) {
    ...
    const loadTransactions = async (accountId: number) => {
        ...
        try {
            ...
            if (matchingAccount?.kind === "connect") {
                const availableCents = connectBalance ?? (await fetchConnectBalanceMinor());
    ...
    const spendingPercentage = totalFlow > 0 ? (totalExpenses / totalFlow) * 100 : 0;
    ...
    const getSuggestion = () => {
        if (netIncome > 0) {
            return {
                icon: "🎉",
                message: "Great job — you earned more than you spent this month! 🎉 Keep it up and consider saving or investing the extra.",
            };
        } else if (spendingPercentage > 80) {
            return {
                icon: "⚠️",
                message: "You're spending close to your income. Consider reviewing your expenses to maintain a healthy budget.",
            };
        } else {
            return {
                icon: "💡",
                message: "You're managing your budget well. Keep tracking your expenses to maintain financial health.",
            };
        }
    };
