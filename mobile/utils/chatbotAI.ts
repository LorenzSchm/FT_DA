...
import { getAccounts, getTransactions, addTransaction } from "./db/finance/finance";
...
const TOOLS = [
        ...
        function: {
            ...
            parameters: {
                ...
                properties: {
                    ...
                    amount: {
                        type: "number",
                        description: "The amount in the main currency unit (e.g. 45.50 for forty-five euros fifty cents)",
                    },
                    description: {
                        type: "string",
                        description: "Short description or merchant name (e.g. 'groceries', 'uber', 'salary')",
                    },
                    category: {
                        type: "string",
    ...
    },
    {
        ...
        function: {
            ...
            description:
                "List active subscriptions for the current account.",
            parameters: {
                type: "object",
                properties: {},
            },
    ...
    try {
        switch (name) {
            case "add_transaction": {
                if (!accountId) return JSON.stringify({ error: "No account selected. Please select an account on the dashboard first." });
                ...
                const signedAmount = txType === "expense" ? -amountMinorAbs : amountMinorAbs;
                ...
                const result = await addTransaction(session.access_token, session.refresh_token, data, accountId);
                return JSON.stringify({ success: true, transaction: result, currency });
            }

            case "get_transactions": {
                if (!accountId) return JSON.stringify({ error: "No account selected." });
                const txData = await getTransactions(session.access_token, session.refresh_token, accountId);
                ...
                transactions = transactions.map((t: any) => ({
                    ...
                    type: t.type || (Number(t.amount_minor || 0) < 0 ? "expense" : "income"),
                }));
                ...
                if (args.filter_type) {
                    transactions = transactions.filter((t: any) => t.type === args.filter_type);
                }

                return JSON.stringify({ transactions: transactions.slice(0, 15), total: transactions.length });
            }

            case "get_balance": {
                const accountsData = await getAccounts(session.access_token, session.refresh_token);
                const accounts = accountsData.rows || accountsData;
                const balances = (accounts || []).map((acc: any) => ({
                    name: acc.name,
                    balance: acc.balance_minor !== undefined ? (acc.balance_minor / 100).toFixed(2) : "0.00",
            ...
            case "get_subscriptions": {
                if (!accountId) return JSON.stringify({ error: "No account selected." });
                const subsData = await getSubscriptions(session.access_token, session.refresh_token, accountId);
            ...
            case "update_transaction": {
                if (!accountId) return JSON.stringify({ error: "No account selected." });
                if (!args.transaction_id) return JSON.stringify({ error: "No transaction ID provided." });
                const updateData: any = {};
                if (args.amount !== undefined) updateData.amount_minor = String(Math.round(args.amount * 100));
...
export async function chat(
    ...
    try {
        ...
        if (!assistantMessage) {
            return { reply: "No response from AI. Please try again.", history: updatedHistory };
        }
        ...
        while (assistantMessage.tool_calls && iterations < MAX_ITERATIONS) {
            ...
            for (const toolCall of assistantMessage.tool_calls) {
                ...
                const toolResult = await executeTool(fnName, fnArgs, session, accountId, accountCurrency);
            ...
            if (!response.ok) {
                return { reply: "⚠️ AI service error while processing tool results.", history: updatedHistory };
            }
            ...
            if (!assistantMessage) {
                return { reply: "No response from AI after tool execution.", history: updatedHistory };
            }
        }
