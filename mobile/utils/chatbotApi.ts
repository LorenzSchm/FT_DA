...
import { ParsedCommand, formatCurrency, formatTransactionList } from "./chatbotParser";
...
async function handleCreate(
    ...
    if (!accountId) {
        return {
            success: false,
            message: "I need an account to add this to. Please select an account on your dashboard first.",
        };
    }
    ...
    if (!cmd.amount || !cmd.amountMinor) {
        return {
            success: false,
            message: "I couldn't detect an amount. Try something like: \"add expense 45 groceries\"",
        };
    }
    ...
    try {
        ...
        const displayAmount = formatCurrency(Math.abs(cmd.amountMinor), cmd.currency);
        return {
            success: true,
...
async function handleRead(
    ...
    if (cmd.entity === "balance" || cmd.raw.toLowerCase().includes("balance")) {
        try {
            ...
            return { success: true, message: `📊 Your account balances:\n\n${lines.join("\n")}` };
        } catch (err: any) {
    ...
    if (cmd.entity === "subscription") {
        if (!accountId) {
            return { success: false, message: "Please select an account first to view subscriptions." };
        }
        try {
            ...
            if (!subs || subs.length === 0) {
                return { success: true, message: "No subscriptions found for this account." };
            }
            const lines = subs.map((s: any) => {
                const active = s.active ? "🟢" : "🔴";
                return `${active} ${s.merchant || s.name}: ${formatCurrency(s.amount_minor || 0, s.currency || "EUR")}`;
            });
            return { success: true, message: `📋 Subscriptions:\n\n${lines.join("\n")}` };
        } catch (err: any) {
            return { success: false, message: `Failed to fetch subscriptions: ${err?.message}` };
        }
    }

    // Default: transactions
    if (!accountId) {
        return { success: false, message: "Please select an account first to view transactions." };
    }
    ...
    try {
        ...
        return { success: false, message: `Failed to fetch transactions: ${err?.message}` };
    }
}
...
async function handleUpdate(
    ...
    if (!cmd.id) {
        return {
            success: false,
            message: "I need a transaction ID to update. Try: \"update transaction id 42 amount to 50\"",
        };
    }
    ...
    if (Object.keys(updateData).length === 0) {
        return {
            success: false,
            message: "I couldn't determine what to update. Try: \"change transaction 42 amount to 50\"",
        };
    }
...
async function handleDelete(
    ...
    if (!cmd.id) {
        return {
            success: false,
            message: "I need a transaction ID to delete. Try: \"delete transaction 67\"",
        };
    }
