...
export const generateToken = async (accessToken:string , refreshToken: string, code: string, name: string) => {
    if (!accessToken || !refreshToken || !code) {
        throw new Error("Missing access token, refresh token or code");
    }
    const resp = await axios.post(`${BASE_URL}/bank/generate-token`, {code, name}, {
        headers: {
